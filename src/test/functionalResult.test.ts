...
async function testConstructors(): Promise<TestSuiteResult> {
  ...
  await runner.run(
    ...
    async () => {
      ...
      assert(
        isSuccess(result) && result.data === 42,
        'Success result should contain correct data'
      );
    }
  );
...
async function testTransformations(): Promise<TestSuiteResult> {
  ...
  await runner.run(
    ...
    async () => {
      ...
      assert(
        doubled.success && doubled.data === 10,
        'Map should transform success values'
      );
      assert(
  ...
  await runner.run('should chain operations correctly', async () => {
    ...
    assert(
      chained.success && chained.data === 15,
      'Chain should transform success values'
    );
    assert(
  ...
  await runner.run(
    ...
    async () => {
      ...
      assert(
        successFold === '42',
        'Fold should return result from success branch'
      );
      assert(
        failureFold === 'error: error',
        'Fold should return result from failure branch'
      );
    }
  );
...
async function testErrorHandling(): Promise<TestSuiteResult> {
  ...
  await runner.run(
    'should tryCatch handle sync functions with type parameters',
    async () => {
...
async function testComposition(): Promise<TestSuiteResult> {
  ...
  await runner.run('should pipe work with curried operations', async () => {
    ...
    assert(
      result.success && result.data === '10',
      'Pipe should work with curried operations'
    );
  });
  ...
  await runner.run(
    ...
    async () => {
      ...
      assert(
        result.success && result.data === '10_processed',
        'Pipe should maintain type safety'
      );
    }
  );
  ...
  await runner.run('should pipe handle point-free style', async () => {
    ...
    assert(
      result.success && result.data === '10',
      'Pipe should handle point-free style'
    );
  });
  ...
  await runner.run('should pipe handle empty operations array', async () => {
    ...
    assert(
      result === initial,
      'Pipe should return initial result for empty operations'
    );
  });
  ...
  await runner.run('should handle failure propagation in pipe', async () => {
    ...
    assert(
      !result.success && result.error === 'error in chain',
      'Pipe should propagate failures'
    );
  });
...
async function testNewFeatures(): Promise<TestSuiteResult> {
  ...
  await runner.run('should sequence handle all success case', async () => {
    ...
    assert(
      sequenced.success &&
        JSON.stringify(sequenced.data) === JSON.stringify([1, 2, 3]),
  ...
  await runner.run(
    ...
    async () => {
      ...
      assert(
        result.success &&
          JSON.stringify(result.data) === JSON.stringify([2, 4, 6]),
  ...
  await runner.run(
    'should getOrElse return default value on failure',
    async () => {
      const successResult = success<number, string>(42);
      const failureResult = failure<number, string>('error');
      ...
      assert(
        successValue === 42,
        'getOrElse should return success value for success results'
      );
      assert(
        failureValue === 0,
        'getOrElse should return default value for failure results'
      );
    }
  );
  ...
  await runner.run('should getOrThrow throw error on failure', async () => {
    ...
    assert(
      successValue === 42,
      'getOrThrow should return success value for success results'
    );
    ...
    try {
      ...
      assert(
        error === 'error',
        'getOrThrow should throw error for failure results'
      );
    }
  });
  ...
  await runner.run('should isSuccess type guard narrow correctly', async () => {
    ...
    if (isSuccess(result)) {
      assert(
        ...
        'isSuccess should narrow to success type'
      );
    } else {
  ...
  await runner.run('should isFailure type guard narrow correctly', async () => {
    ...
    if (isFailure(result)) {
      assert(
        ...
        'isFailure should narrow to error type'
      );
    } else {
...
async function testEdgeCases(): Promise<TestSuiteResult> {
  ...
  await runner.run('should handle nested Result types', async () => {
    ...
    assert(
      flattened.success && flattened.data === 'nested',
  ...
  await runner.run('should handle empty arrays in sequence', async () => {
    ...
    assert(
      ...
      'Sequence should handle empty arrays'
    );
  });
  ...
  await runner.run('should handle empty arrays in traverse', async () => {
    ...
    assert(
      result.success && result.data.length === 0,
      'Traverse should handle empty arrays'
    );
  });
